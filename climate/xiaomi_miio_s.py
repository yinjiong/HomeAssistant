...
class XiaomiAirConditioningCompanion(ClimateDevice):
        ...
        try:
            ...
            if len(self._autoSwitch) == 0 or self.hass.states.get(self._autoSwitch).state == 'off':
                ...
                self._state_attrs.update({
                    ...
                    ATTR_SWING_MODE: self.swing_mode_cn[state.swing_mode.name],
                    ATTR_FAN_SPEED: self.fan_mode_cn[state.fan_speed.name],
                    ATTR_OPERATION_MODE: self.operation_cn[state.mode.name],
                    ATTR_LED: state.led,
                })            
                _current_operation_cn = self.operation_cn[state.mode.name]
                self._current_operation = _current_operation_cn
                self._target_temperature = state.target_temperature
                _current_fan_mode_cn = self.fan_mode_cn[state.fan_speed.name]
                self._current_fan_mode = _current_fan_mode_cn
                _current_swing_mode_cn = self.swing_mode_cn[state.swing_mode.name]
                self._current_swing_mode = _current_swing_mode_cn
            else:
    ...
    def operation_list(self):
        ...
        return [self.operation_cn[mode.name] for mode in OperationMode]
    ...
    def fan_list(self):
        ...
        return [self.fan_mode_cn[speed.name] for speed in FanSpeed]
    ...
    def async_set_swing_mode(self, swing_mode):
        ...
        # self._current_swing_mode = SwingMode[list(self.swing_mode_cn.keys())[list(self.swing_mode_cn.values()).index(swing_mode.title())]]
        yield from self._send_configuration()
    ...
    def async_set_fan_mode(self, fan_mode):
        ...
        # self._current_fan_mode = FanSpeed[list(self.fan_mode_cn.keys())[list(self.fan_mode_cn.values()).index(fan.title())]]
        yield from self._send_configuration()
    ...
    def async_set_operation_mode(self, operation_mode):
        ...
        # self._current_operation = OperationMode[list(self.operation_cn.keys())[list(self.operation_cn.values()).index(operation_mode.title())]]
        yield from self._send_configuration()
    ...
    def swing_list(self):
        ...
        from miio.airconditioningcompanion import SwingMode
        return [self.swing_mode_cn[mode.name] for mode in SwingMode]
    ...
    def _send_configuration(self):
        ...
        if self._air_condition_model is not None:
            yield from self._try_command(
                ...
                OperationMode[list(self.operation_cn.keys())[list(self.operation_cn.values()).index(self._current_operation)]],
                int(self._target_temperature),
                FanSpeed[list(self.fan_mode_cn.keys())[list(self.fan_mode_cn.values()).index(self._current_fan_mode)]],
                SwingMode[list(self.swing_mode_cn.keys())[list(self.swing_mode_cn.values()).index(self._current_swing_mode)]],
                Led.Off,
            )
        else:
            ...
            if len(command['mode']) > 0:
                if command['mode'].find("off") == 0:
                    ...
                    self._state_attrs.update({
                        ...
                        ATTR_FAN_SPEED: command['fan'],
                        ATTR_OPERATION_MODE: command['mode'],
                        ATTR_LED: self.led,
                    }) 
